...
export default function CivicShieldApp({ onWipe }) {
  ...
  // Polling check to automatically unlock when approved
  useEffect(() => {
    const checkZkp = () => {
      const commitment = sessionStorage.getItem('zkp_commitment')
      setIsZkpVerified(!!commitment)
    }
    checkZkp()
    const interval = setInterval(checkZkp, 1000)
    return () => clearInterval(interval)
  }, [])

  // Redirect to wallet if unverified
  useEffect(() => {
    if (!isZkpVerified) {
      setActiveTab('wallet')
    }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col max-w-md mx-auto">
      ...
      <main className="flex-1 overflow-y-auto">
        {activeTab === 'home' && isZkpVerified && <HomeTab />}
        {activeTab === 'report' && isZkpVerified && <ReportScreen />}
        {(activeTab === 'wallet' || !isZkpVerified) && <div className="p-4"><ZKPWallet /></div>}
      </main>
      ...
      <nav className="border-t border-gray-200 bg-white sticky bottom-0 z-10">
        <div className="flex">
          {TABS.map(tab => {
            const isLocked = !isZkpVerified && tab.key !== 'wallet'
            return (
              <button
                key={tab.key}
                id={`tab-${tab.key}`}
                ...
                className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 text-xs font-medium transition-colors ${
                  isLocked ? 'opacity-40 cursor-not-allowed text-gray-300' :
                  activeTab === tab.key
                    ? 'text-blue-600 border-t-2 border-blue-600'
                    : 'text-gray-400 border-t-2 border-transparent hover:text-gray-600'
                }`}
              >
                <span className="text-base">{isLocked ? '🔒' : tab.icon}</span>
                {tab.label}
              </button>
            )
          })}
